...
<?php if ($Page->service_id->Visible) { // service_id ?>
    <div id="r_service_id"<?= $Page->service_id->rowAttributes() ?>>
        <label id="elh_jdh_services_service_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->service_id->caption() ?><?= $Page->service_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->service_id->getDisplayValue($Page->service_id->EditValue))) ?>"></span>
<input type="hidden" data-table="jdh_services" data-field="x_service_id" data-hidden="1" name="x_service_id" id="x_service_id" value="<?= HtmlEncode($Page->service_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
loadjs.ready("fjdh_servicesedit", function() {
    ...
    if (fjdh_servicesedit.lists.category_id?.lookupOptions.length) {
        options.data = { id: "x_category_id", form: "fjdh_servicesedit" };
    } else {
        options.ajax = { id: "x_category_id", form: "fjdh_servicesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready("fjdh_servicesedit", function() {
    ...
    if (fjdh_servicesedit.lists.subcategory_id?.lookupOptions.length) {
        options.data = { id: "x_subcategory_id", form: "fjdh_servicesedit" };
    } else {
        options.ajax = { id: "x_subcategory_id", form: "fjdh_servicesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php if ($Page->service_description->Visible) { // service_description ?>
    <div id="r_service_description"<?= $Page->service_description->rowAttributes() ?>>
        <label id="elh_jdh_services_service_description" class="<?= $Page->LeftColumnClass ?>"><?= $Page->service_description->caption() ?><?= $Page->service_description->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->service_description->cellAttributes() ?>>
<span id="el_jdh_services_service_description">
<?php $Page->service_description->EditAttrs->appendClass("editor"); ?>
...
<script>
loadjs.ready(["fjdh_servicesedit", "editor"], function() {
    ew.createEditor("fjdh_servicesedit", "x_service_description", 35, 4, <?= $Page->service_description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fjdh_servicesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fjdh_servicesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
