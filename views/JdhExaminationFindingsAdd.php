...
// Page object
$JdhExaminationFindingsAdd = &$Page;
?>
<script>
...
var currentForm;
var fjdh_examination_findingsadd;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fjdh_examination_findingsadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="fjdh_examination_findingsadd" id="fjdh_examination_findingsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
...
<?php if ($Page->patient_id->Visible) { // patient_id ?>
    <div id="r_patient_id"<?= $Page->patient_id->rowAttributes() ?>>
        <label id="elh_jdh_examination_findings_patient_id" for="x_patient_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->patient_id->caption() ?><?= $Page->patient_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<span id="el_jdh_examination_findings_patient_id">
    <select
        ...
        data-select2-id="fjdh_examination_findingsadd_x_patient_id"
...
<script>
loadjs.ready("fjdh_examination_findingsadd", function() {
    var options = { name: "x_patient_id", selectId: "fjdh_examination_findingsadd_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_examination_findingsadd.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fjdh_examination_findingsadd" };
    } else {
        options.ajax = { id: "x_patient_id", form: "fjdh_examination_findingsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fjdh_examination_findingsadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fjdh_examination_findingsadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
