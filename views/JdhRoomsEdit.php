...
?>
<main class="edit">
<form name="fjdh_roomsedit" id="fjdh_roomsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
...
<?php if ($Page->room_id->Visible) { // room_id ?>
    <div id="r_room_id"<?= $Page->room_id->rowAttributes() ?>>
        <label id="elh_jdh_rooms_room_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->room_id->caption() ?><?= $Page->room_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->room_id->getDisplayValue($Page->room_id->EditValue))) ?>"></span>
<input type="hidden" data-table="jdh_rooms" data-field="x_room_id" data-hidden="1" name="x_room_id" id="x_room_id" value="<?= HtmlEncode($Page->room_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_jdh_rooms_ward_id">
    <select
        ...
        data-select2-id="fjdh_roomsedit_x_ward_id"
...
<script>
loadjs.ready("fjdh_roomsedit", function() {
    var options = { name: "x_ward_id", selectId: "fjdh_roomsedit_x_ward_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_roomsedit.lists.ward_id?.lookupOptions.length) {
        options.data = { id: "x_ward_id", form: "fjdh_roomsedit" };
    } else {
        options.ajax = { id: "x_ward_id", form: "fjdh_roomsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fjdh_roomsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fjdh_roomsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
