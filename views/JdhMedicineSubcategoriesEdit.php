...
// Page object
$JdhMedicineSubcategoriesEdit = &$Page;
?>
<script>
...
?>
<main class="edit">
<form name="fjdh_medicine_subcategoriesedit" id="fjdh_medicine_subcategoriesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { jdh_medicine_subcategories: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fjdh_medicine_subcategoriesedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fjdh_medicine_subcategoriesedit")
        .setPageId("edit")
...
<?php if ($Page->subcategory_id->Visible) { // subcategory_id ?>
    <div id="r_subcategory_id"<?= $Page->subcategory_id->rowAttributes() ?>>
        <label id="elh_jdh_medicine_subcategories_subcategory_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->subcategory_id->caption() ?><?= $Page->subcategory_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="jdh_medicine_subcategories" data-field="x_subcategory_id" data-hidden="1" name="x_subcategory_id" id="x_subcategory_id" value="<?= HtmlEncode($Page->subcategory_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_jdh_medicine_subcategories_category_id">
    <select
        ...
        data-select2-id="fjdh_medicine_subcategoriesedit_x_category_id"
...
<script>
loadjs.ready("fjdh_medicine_subcategoriesedit", function() {
    var options = { name: "x_category_id", selectId: "fjdh_medicine_subcategoriesedit_x_category_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_medicine_subcategoriesedit.lists.category_id?.lookupOptions.length) {
        options.data = { id: "x_category_id", form: "fjdh_medicine_subcategoriesedit" };
    } else {
        options.ajax = { id: "x_category_id", form: "fjdh_medicine_subcategoriesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fjdh_medicine_subcategoriesedit", "editor"], function() {
    ew.createEditor("fjdh_medicine_subcategoriesedit", "x_description", 35, 4, <?= $Page->description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fjdh_medicine_subcategoriesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fjdh_medicine_subcategoriesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
