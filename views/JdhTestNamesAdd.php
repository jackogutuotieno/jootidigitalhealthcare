...
$Page->showMessage();
?>
<form name="fjdh_test_namesadd" id="fjdh_test_namesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
...
<span id="el_jdh_test_names_patient_id">
    <select
        ...
        data-select2-id="fjdh_test_namesadd_x_patient_id"
...
<script>
loadjs.ready("fjdh_test_namesadd", function() {
    var options = { name: "x_patient_id", selectId: "fjdh_test_namesadd_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_test_namesadd.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fjdh_test_namesadd" };
    } else {
        options.ajax = { id: "x_patient_id", form: "fjdh_test_namesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready("fjdh_test_namesadd", function() {
    fjdh_test_namesadd.createAutoSuggest(Object.assign({"id":"x_test_category_id","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->test_category_id->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.jdh_test_names.fields.test_category_id.autoSuggestOptions));
});
</script>
...
<span id="el_jdh_test_names_test_subcategory_id">
    <select
        ...
        data-select2-id="fjdh_test_namesadd_x_test_subcategory_id"
        data-table="jdh_test_names"
...
<script>
loadjs.ready("fjdh_test_namesadd", function() {
    var options = { name: "x_test_subcategory_id", selectId: "fjdh_test_namesadd_x_test_subcategory_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_test_namesadd.lists.test_subcategory_id?.lookupOptions.length) {
        options.data = { id: "x_test_subcategory_id", form: "fjdh_test_namesadd" };
    } else {
        options.ajax = { id: "x_test_subcategory_id", form: "fjdh_test_namesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fjdh_test_namesadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fjdh_test_namesadd", "x_date_submitted", jQuery.extend(true, {"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fjdh_test_namesadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fjdh_test_namesadd", "x_date_updated", jQuery.extend(true, {"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fjdh_test_namesadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fjdh_test_namesadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
