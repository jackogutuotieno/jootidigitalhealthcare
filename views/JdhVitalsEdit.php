...
<span id="el_jdh_vitals_patient_id">
    <select
        ...
        data-select2-id="fjdh_vitalsedit_x_patient_id"
...
<script>
loadjs.ready("fjdh_vitalsedit", function() {
    var options = { name: "x_patient_id", selectId: "fjdh_vitalsedit_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_vitalsedit.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fjdh_vitalsedit" };
    } else {
        options.ajax = { id: "x_patient_id", form: "fjdh_vitalsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<textarea data-table="jdh_vitals" data-field="x_patient_status" name="x_patient_status" id="x_patient_status" cols="35" rows="4" placeholder="<?= HtmlEncode($Page->patient_status->getPlaceHolder()) ?>"<?= $Page->patient_status->editAttributes() ?> aria-describedby="x_patient_status_help"><?= $Page->patient_status->EditValue ?></textarea>
