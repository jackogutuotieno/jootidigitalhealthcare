...
// Page object
$JdhPrescriptionsActionsAdd = &$Page;
?>
<script>
...
var currentForm;
var fjdh_prescriptions_actionsadd;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fjdh_prescriptions_actionsadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="fjdh_prescriptions_actionsadd" id="fjdh_prescriptions_actionsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_jdh_prescriptions_actions_medicine_id">
    <select
        ...
        data-select2-id="fjdh_prescriptions_actionsadd_x_medicine_id"
        <?php } ?>
...
<?php if (!$Page->medicine_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fjdh_prescriptions_actionsadd", function() {
    var options = { name: "x_medicine_id", selectId: "fjdh_prescriptions_actionsadd_x_medicine_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_prescriptions_actionsadd.lists.medicine_id?.lookupOptions.length) {
        options.data = { id: "x_medicine_id", form: "fjdh_prescriptions_actionsadd" };
    } else {
        options.ajax = { id: "x_medicine_id", form: "fjdh_prescriptions_actionsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_jdh_prescriptions_actions_patient_id">
    <select
        ...
        data-select2-id="fjdh_prescriptions_actionsadd_x_patient_id"
        <?php } ?>
        data-table="jdh_prescriptions_actions"
...
<script>
loadjs.ready("fjdh_prescriptions_actionsadd", function() {
    var options = { name: "x_patient_id", selectId: "fjdh_prescriptions_actionsadd_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_prescriptions_actionsadd.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fjdh_prescriptions_actionsadd" };
    } else {
        options.ajax = { id: "x_patient_id", form: "fjdh_prescriptions_actionsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fjdh_prescriptions_actionsadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fjdh_prescriptions_actionsadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
