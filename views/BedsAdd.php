...
$Page->showMessage();
?>
<form name="fbedsadd" id="fbedsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
...
<span id="el_beds_room_id">
    <select
        ...
        data-select2-id="fbedsadd_x_room_id"
        data-table="beds"
...
<script>
loadjs.ready("fbedsadd", function() {
    var options = { name: "x_room_id", selectId: "fbedsadd_x_room_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbedsadd.lists.room_id?.lookupOptions.length) {
        options.data = { id: "x_room_id", form: "fbedsadd" };
    } else {
        options.ajax = { id: "x_room_id", form: "fbedsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_beds_status_id">
    <select
        ...
        data-select2-id="fbedsadd_x_status_id"
        data-table="beds"
...
<script>
loadjs.ready("fbedsadd", function() {
    var options = { name: "x_status_id", selectId: "fbedsadd_x_status_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbedsadd.lists.status_id?.lookupOptions.length) {
        options.data = { id: "x_status_id", form: "fbedsadd" };
    } else {
        options.ajax = { id: "x_status_id", form: "fbedsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
