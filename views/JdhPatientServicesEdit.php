...
?>
<main class="edit">
<form name="fjdh_patient_servicesedit" id="fjdh_patient_servicesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { jdh_patient_services: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fjdh_patient_servicesedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fjdh_patient_servicesedit")
        .setPageId("edit")
...
<?php if ($Page->patient_service_id->Visible) { // patient_service_id ?>
    <div id="r_patient_service_id"<?= $Page->patient_service_id->rowAttributes() ?>>
        <label id="elh_jdh_patient_services_patient_service_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->patient_service_id->caption() ?><?= $Page->patient_service_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->patient_service_id->cellAttributes() ?>>
...
<input type="hidden" data-table="jdh_patient_services" data-field="x_patient_service_id" data-hidden="1" name="x_patient_service_id" id="x_patient_service_id" value="<?= HtmlEncode($Page->patient_service_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_jdh_patient_services_patient_id">
    <select
        ...
        data-select2-id="fjdh_patient_servicesedit_x_patient_id"
...
<script>
loadjs.ready("fjdh_patient_servicesedit", function() {
    var options = { name: "x_patient_id", selectId: "fjdh_patient_servicesedit_x_patient_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_patient_servicesedit.lists.patient_id?.lookupOptions.length) {
        options.data = { id: "x_patient_id", form: "fjdh_patient_servicesedit" };
    } else {
        options.ajax = { id: "x_patient_id", form: "fjdh_patient_servicesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_jdh_patient_services_service_id">
    <select
        ...
        data-select2-id="fjdh_patient_servicesedit_x_service_id"
        data-table="jdh_patient_services"
...
<script>
loadjs.ready("fjdh_patient_servicesedit", function() {
    var options = { name: "x_service_id", selectId: "fjdh_patient_servicesedit_x_service_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_patient_servicesedit.lists.service_id?.lookupOptions.length) {
        options.data = { id: "x_service_id", form: "fjdh_patient_servicesedit" };
    } else {
        options.ajax = { id: "x_service_id", form: "fjdh_patient_servicesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fjdh_patient_servicesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fjdh_patient_servicesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
