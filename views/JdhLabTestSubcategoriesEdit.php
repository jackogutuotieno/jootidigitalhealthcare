...
// Page object
$JdhLabTestSubcategoriesEdit = &$Page;
?>
...
?>
<main class="edit">
<form name="fjdh_lab_test_subcategoriesedit" id="fjdh_lab_test_subcategoriesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { jdh_lab_test_subcategories: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fjdh_lab_test_subcategoriesedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fjdh_lab_test_subcategoriesedit")
        .setPageId("edit")
...
<?php if ($Page->test_subcategory_id->Visible) { // test_subcategory_id ?>
    <div id="r_test_subcategory_id"<?= $Page->test_subcategory_id->rowAttributes() ?>>
        <label id="elh_jdh_lab_test_subcategories_test_subcategory_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->test_subcategory_id->caption() ?><?= $Page->test_subcategory_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->test_subcategory_id->cellAttributes() ?>>
<span id="el_jdh_lab_test_subcategories_test_subcategory_id">
<span<?= $Page->test_subcategory_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->test_subcategory_id->getDisplayValue($Page->test_subcategory_id->EditValue))) ?>"></span>
<input type="hidden" data-table="jdh_lab_test_subcategories" data-field="x_test_subcategory_id" data-hidden="1" name="x_test_subcategory_id" id="x_test_subcategory_id" value="<?= HtmlEncode($Page->test_subcategory_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_jdh_lab_test_subcategories_test_category_id">
    <select
        ...
        data-select2-id="fjdh_lab_test_subcategoriesedit_x_test_category_id"
        <?php } ?>
...
<?php if (!$Page->test_category_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fjdh_lab_test_subcategoriesedit", function() {
    var options = { name: "x_test_category_id", selectId: "fjdh_lab_test_subcategoriesedit_x_test_category_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjdh_lab_test_subcategoriesedit.lists.test_category_id?.lookupOptions.length) {
        options.data = { id: "x_test_category_id", form: "fjdh_lab_test_subcategoriesedit" };
    } else {
        options.ajax = { id: "x_test_category_id", form: "fjdh_lab_test_subcategoriesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fjdh_lab_test_subcategoriesedit", "editor"], function() {
    ew.createEditor("fjdh_lab_test_subcategoriesedit", "x_description", 35, 4, <?= $Page->description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fjdh_lab_test_subcategoriesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fjdh_lab_test_subcategoriesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
