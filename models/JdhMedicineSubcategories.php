...
/**
 * Table class for jdh_medicine_subcategories
 */
class JdhMedicineSubcategories extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "`jdh_medicine_subcategories`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->subcategory_id->IsAutoIncrement = true; // Autoincrement field
        $this->subcategory_id->IsPrimaryKey = true; // Primary key field
        $this->subcategory_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->subcategory_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        // subcategory_name $tbl, $fldvar, $fldname, $fldexp, $fldbsexp, $fldtype, $fldsize, $flddtfmt, $upload, $fldvirtualexp, $fldvirtual, $forceselect, $fldvirtualsrch, $fldviewtag = "", $fldhtmltag
        $this->subcategory_name = new DbField(
            $this, // Table
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`jdh_medicine_subcategories`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->subcategory_id->setDbValue($conn->lastInsertId());
            $rs['subcategory_id'] = $this->subcategory_id->DbValue;
        }
        return $success;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['subcategory_id']) && !EmptyValue($this->subcategory_id->CurrentValue)) {
                $rs['subcategory_id'] = $this->subcategory_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('subcategory_id', $rs)) {
                AddFilter($where, QuotedName('subcategory_id', $this->Dbid) . '=' . QuotedValue($rs['subcategory_id'], $this->subcategory_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->subcategory_id->CurrentValue : $this->subcategory_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->subcategory_id->CurrentValue = $keys[0];
            } else {
                $this->subcategory_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('subcategory_id', $row) ? $row['subcategory_id'] : null;
        } else {
            $val = !EmptyValue($this->subcategory_id->OldValue) && !$current ? $this->subcategory_id->OldValue : $this->subcategory_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@subcategory_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("jdhmedicinesubcategorieslist");
    }
    ...
    {
        global $Language;
        if ($pageName == "jdhmedicinesubcategoriesview") {
            return $Language->phrase("View");
        } elseif ($pageName == "jdhmedicinesubcategoriesedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "jdhmedicinesubcategoriesadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "JdhMedicineSubcategoriesView";
            case Config("API_ADD_ACTION"):
                return "JdhMedicineSubcategoriesAdd";
            case Config("API_EDIT_ACTION"):
                return "JdhMedicineSubcategoriesEdit";
            case Config("API_DELETE_ACTION"):
                return "JdhMedicineSubcategoriesDelete";
            case Config("API_LIST_ACTION"):
                return "JdhMedicineSubcategoriesList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jdhmedicinesubcategoriesview", $parm);
        } else {
            $url = $this->keyUrl("jdhmedicinesubcategoriesview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "jdhmedicinesubcategoriesadd?" . $parm;
        } else {
            $url = "jdhmedicinesubcategoriesadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhmedicinesubcategoriesedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhmedicinesubcategorieslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhmedicinesubcategoriesadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhmedicinesubcategorieslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jdhmedicinesubcategoriesdelete");
        }
    }
    ...
    {
        $json = "";
        $json .= "\"subcategory_id\":" . JsonEncode($this->subcategory_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->subcategory_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->subcategory_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("subcategory_id") ?? Route("subcategory_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->subcategory_id->CurrentValue = $key;
            } else {
                $this->subcategory_id->OldValue = $key;
            }
    ...
    {
        global $Response;
        $listPage = "JdhMedicineSubcategoriesList";
    ...
    public function renderEditRow()
    {
        ...
        // subcategory_name
        $this->subcategory_name->setupEditAttributes();
