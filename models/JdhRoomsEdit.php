 ...
 * Page class
 */
class JdhRoomsEdit extends JdhRooms
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhRoomsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "jdhroomsedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("room_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->room_id->setQueryStringValue($keyValue);
                $this->room_id->setOldValue($this->room_id->QueryStringValue);
            } elseif (Post("room_id") !== null) {
                $this->room_id->setFormValue(Post("room_id"));
                $this->room_id->setOldValue($this->room_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("room_id") ?? Route("room_id")) !== null) {
                    $this->room_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("jdhroomslist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "jdhroomslist") {
                ...
                if ($this->editRow()) {
                    ...
                    if ($this->UseAjaxActions && GetPageName($returnUrl) != "jdhroomslist") {
                        Container("flash")->addMessage("Return-Url", $returnUrl); // Save return URL
                        $returnUrl = "jdhroomslist"; // Return list page content
                    }
    ...
    public function renderRow()
    {
        ...
        // room_number
        $this->room_number->RowCssClass = "row";
    ...
    protected function editRow()
    {
        ...
        // room_number
        $this->room_number->setDbValueDef($rsnew, $this->room_number->CurrentValue, 0, $this->room_number->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("jdhroomslist"), "", $this->TableVar, true);
        $pageId = "edit";
