...
class JdhUsersList extends JdhUsers
{
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fjdh_userssrch");
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fjdh_userssrch", $filters);
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions as $listAction) {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $profile = UserProfile::create()->setUserName(strval($this->email_address->CurrentValue))
                        ->load(HtmlDecode($this->biography->CurrentValue));
                    if ($action == "forcelogoutuser") {
                        ...
                        if ($totalCount == 0 && $activeCount == 0) { // Do not show link if no active sessions
                            ...
                            $this->email_address->ViewValue .= '&nbsp;<span class="badge rounded-pill text-bg-info">' . $message . '</span>';
                        }
                    }
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fjdh_userslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttrs() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fjdh_userslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttrs() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_jdh_users", "data-rowtype" => RowType::ADD]);
