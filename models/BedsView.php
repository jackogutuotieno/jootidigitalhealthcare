...
class BedsView extends Beds
{
    ...
    // CSS class/style
    public $CurrentPageName = "bedsview";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("bed_id") ?? Route("bed_id")) !== null) {
            ...
            $this->RecKey["bed_id"] = $this->bed_id->QueryStringValue;
        } elseif (Post("bed_id") !== null) {
            $this->bed_id->setFormValue(Post("bed_id"));
            $this->RecKey["bed_id"] = $this->bed_id->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->bed_id->setQueryStringValue($keyValue);
            $this->RecKey["bed_id"] = $this->bed_id->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "bedslist"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "bedslist"; // No matching record, return to list
                    }
                break;
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fbedsview" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-key="' . HtmlEncode(ArrayToJsonAttribute($this->RecKey)) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
