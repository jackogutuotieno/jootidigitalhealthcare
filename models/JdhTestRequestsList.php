 ...
 * Page class
 */
class JdhTestRequestsList extends JdhTestRequests
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhTestRequestsList";
    ...
    public $FormName = "fjdh_test_requestslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "jdhtestrequestslist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "jdhtestrequestsdelete";
        $this->MultiUpdateUrl = "jdhtestrequestsupdate";
    ...
    {
        if (!SameString($this->request_id->OldValue, $this->request_id->CurrentValue)) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->request_description->clearErrorMessage();
        $this->request_date->clearErrorMessage();
    }
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fjdh_test_requestssrch", $filters);
    ...
    public function renderListOptions()
    {
        ...
        if ($this->isInlineAddRow() || $this->isInlineCopyRow()) { // Inline Add/Copy
            ...
            if ($this->UseAjaxActions) {
                ...
                $opt->Body = <<<INLINEADD
                    <div{$divClass}>
                        <button class="ew-grid-link ew-inline-insert" title="{$insertTitle}" data-caption="{$insertTitle}" form="fjdh_test_requestslist" formaction="{$inlineInsertUrl}">{$insertCaption}</button>
        ...
        if ($this->isInlineEditRow()) { // Inline-Edit
            ...
            $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
                ...
                if ($this->UseAjaxActions) {
                    ...
                    $opt->Body = <<<INLINEEDIT
                    <div{$divClass}>
                        <button class="ew-grid-link ew-inline-update" title="{$updateTitle}" data-caption="{$updateTitle}" form="fjdh_test_requestslist" formaction="{$inlineUpdateUrl}">{$updateCaption}</button>
            ...
            $opt->Body .= "<input type=\"hidden\" name=\"" . $this->OldKeyName . "\" id=\"" . $this->OldKeyName . "\" value=\"" . HtmlEncode($this->request_id->CurrentValue) . "\">";
            return;
        }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fjdh_test_requestslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fjdh_test_requestslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->request_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fjdh_test_requestslist\"" .
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fjdh_test_requestssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fjdh_test_requestssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fjdh_test_requestslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if (($this->isGridAdd() || $this->isGridEdit())) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_jdh_test_requests", "data-rowtype" => ROWTYPE_ADD]);
    ...
    protected function editRow()
    {
        ...
        // request_date
        $this->request_date->setDbValueDef($rsnew, UnFormatDateTime($this->request_date->CurrentValue, $this->request_date->formatPattern()), CurrentDate(), $this->request_date->ReadOnly);
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['request_title']); // request_title
        $hash .= GetFieldHash($row['request_service_id']); // request_service_id
        $hash .= GetFieldHash($row['request_description']); // request_description
        $hash .= GetFieldHash($row['request_date']); // request_date
        return md5($hash);
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // request_service_id
        $this->request_service_id->setDbValueDef($rsnew, $this->request_service_id->CurrentValue, 0, false);

        // request_description
        $this->request_description->setDbValueDef($rsnew, $this->request_description->CurrentValue, null, false);

        // request_date
        $this->request_date->setDbValueDef($rsnew, UnFormatDateTime($this->request_date->CurrentValue, $this->request_date->formatPattern()), CurrentDate(), false);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fjdh_test_requestslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fjdh_test_requestssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
