...
/**
 * Table class for jdh_medicines
 */
class JdhMedicines extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "jdh_medicines";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->category_id->Lookup = new Lookup($this->category_id, 'jdh_medicine_categories', false, 'category_id', ["category_name","","",""], '', '', [], [], [], [], [], [], false, '', '', "`category_name`");
        ...
        // buying_price
        $this->buying_price = new DbField(
            $this, // Table
            'x_buying_price', // Variable name
            'buying_price', // Name
            '`buying_price`', // Expression
            '`buying_price`', // Basic search expression
            5, // Type
            22, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`buying_price`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->buying_price->InputTextType = "text";
        $this->buying_price->Raw = true;
        $this->buying_price->Nullable = false; // NOT NULL field
        $this->buying_price->Required = true; // Required field
        $this->buying_price->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->buying_price->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['buying_price'] = &$this->buying_price;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "jdh_medicines";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("jdhmedicineslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "jdhmedicinesview" => $Language->phrase("View"),
            "jdhmedicinesedit" => $Language->phrase("Edit"),
            "jdhmedicinesadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "JdhMedicinesView",
            Config("API_ADD_ACTION") => "JdhMedicinesAdd",
            Config("API_EDIT_ACTION") => "JdhMedicinesEdit",
            Config("API_DELETE_ACTION") => "JdhMedicinesDelete",
            Config("API_LIST_ACTION") => "JdhMedicinesList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jdhmedicinesview", $parm);
        } else {
            $url = $this->keyUrl("jdhmedicinesview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhmedicinesedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhmedicineslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhmedicinesadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhmedicineslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jdhmedicinesdelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->expiry->EditValue = FormatDateTime($this->expiry->CurrentValue, $this->expiry->formatPattern());
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM jdh_medicines";
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'jdh_medicines');
    }
