 ...
 * Page class
 */
class JdhServiceSubcategoryEdit extends JdhServiceSubcategory
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhServiceSubcategoryEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "jdhservicesubcategoryedit";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["jdh_service_subcategory"]) || $GLOBALS["jdh_service_subcategory"]::class == PROJECT_NAMESPACE . "jdh_service_subcategory") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (
                    ...
                    if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                        ...
                        $result["view"] = SameString($pageName, "jdhservicesubcategoryview"); // If View page, no primary button
                    } else { // List page
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("subcategory_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->subcategory_id->setQueryStringValue($keyValue);
                $this->subcategory_id->setOldValue($this->subcategory_id->QueryStringValue);
            } elseif (Post("subcategory_id") !== null) {
                $this->subcategory_id->setFormValue(Post("subcategory_id"));
                $this->subcategory_id->setOldValue($this->subcategory_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("subcategory_id") ?? Route("subcategory_id")) !== null) {
                    $this->subcategory_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->subcategory_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("jdhservicesubcategorylist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "jdhservicesubcategorylist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "jdhservicesubcategorylist") {
                            ...
                            $returnUrl = "jdhservicesubcategorylist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    public function renderRow()
    {
        ...
        // subcategory_name
        $this->subcategory_name->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->subcategory_name->Visible && $this->subcategory_name->Required) {
                if (!$this->subcategory_name->IsDetailKey && EmptyValue($this->subcategory_name->FormValue)) {
                    $this->subcategory_name->addErrorMessage(str_replace("%s", $this->subcategory_name->caption(), $this->subcategory_name->RequiredErrorMessage));
                }
            }
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (description)
        if ($this->description->CurrentValue != "") {
            $filterChk = "(`description` = '" . AdjustSql($this->description->CurrentValue, $this->Dbid) . "')";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->description->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->description->CurrentValue, $idxErrMsg);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['subcategory_name'])) { // subcategory_name
            $this->subcategory_name->CurrentValue = $row['subcategory_name'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("jdhservicesubcategorylist"), "", $this->TableVar, true);
        $pageId = "edit";
