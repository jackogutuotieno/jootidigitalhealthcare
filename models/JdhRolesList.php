 ...
 * Page class
 */
class JdhRolesList extends JdhRoles
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhRolesList";
    ...
    public $FormName = "fjdh_roleslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "jdhroleslist";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'jdh_roles';
        $this->TableName = 'jdh_roles';
        ...
        // Table object (jdh_roles)
        if (!isset($GLOBALS["jdh_roles"]) || get_class($GLOBALS["jdh_roles"]) == PROJECT_NAMESPACE . "jdh_roles") {
            $GLOBALS["jdh_roles"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "jdhrolesadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "jdhrolesdelete";
        $this->MultiUpdateUrl = "jdhrolesupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'jdh_roles');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page => View page
                    ...
                    $result["view"] = $pageName == "jdhrolesview"; // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        $this->role_name->setVisibility();
        $this->role_description->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fjdh_rolessrch");
        }
        $filterList = Concat($filterList, $this->role_id->AdvancedSearch->toJson(), ","); // Field role_id
        $filterList = Concat($filterList, $this->role_name->AdvancedSearch->toJson(), ","); // Field role_name
        $filterList = Concat($filterList, $this->role_description->AdvancedSearch->toJson(), ","); // Field role_description
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fjdh_rolessrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field role_name
        $this->role_name->AdvancedSearch->SearchValue = @$filter["x_role_name"];
        $this->role_name->AdvancedSearch->SearchOperator = @$filter["z_role_name"];
        $this->role_name->AdvancedSearch->SearchCondition = @$filter["v_role_name"];
        $this->role_name->AdvancedSearch->SearchValue2 = @$filter["y_role_name"];
        $this->role_name->AdvancedSearch->SearchOperator2 = @$filter["w_role_name"];
        $this->role_name->AdvancedSearch->save();

        // Field role_description
        $this->role_description->AdvancedSearch->SearchValue = @$filter["x_role_description"];
        $this->role_description->AdvancedSearch->SearchOperator = @$filter["z_role_description"];
        $this->role_description->AdvancedSearch->SearchCondition = @$filter["v_role_description"];
        $this->role_description->AdvancedSearch->SearchValue2 = @$filter["y_role_description"];
        $this->role_description->AdvancedSearch->SearchOperator2 = @$filter["w_role_description"];
        $this->role_description->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->role_name;
        $searchFlds[] = &$this->role_description;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->role_name); // role_name
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->role_name->setSort("");
                $this->role_description->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"jdh_roles\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"jdh_roles\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"jdh_roles\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fjdh_roleslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fjdh_roleslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->role_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"jdh_roles\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fjdh_roleslist\"" .
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("role_id", $this->createColumnOption("role_id"));
            $option->add("role_name", $this->createColumnOption("role_name"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fjdh_rolessrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fjdh_rolessrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fjdh_roleslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if (($this->isGridAdd() || $this->isGridEdit())) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_jdh_roles", "data-rowtype" => ROWTYPE_ADD]);
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->role_name->setDbValue($row['role_name']);
        $this->role_description->setDbValue($row['role_description']);
    }
    ...
    protected function newRow()
    {
        ...
        $row['role_name'] = $this->role_name->DefaultValue;
        $row['role_description'] = $this->role_description->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // role_name
            $this->role_name->ViewValue = $this->role_name->CurrentValue;
            ...
            // role_name
            $this->role_name->HrefValue = "";
            $this->role_name->TooltipValue = "";
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fjdh_roleslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fjdh_rolessrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
