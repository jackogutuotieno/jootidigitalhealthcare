 ...
 * Page class
 */
class JdhBrandingAdd extends JdhBranding
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhBrandingAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "jdhbrandingadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("jdhbrandinglist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "jdhbrandingview") {
    ...
    protected function getUploadFiles()
    {
        ...
        $this->header_image->Upload->Index = $CurrentForm->Index;
        $this->header_image->Upload->uploadFile();
        $this->footer_image->Upload->Index = $CurrentForm->Index;
        $this->footer_image->Upload->uploadFile();
    }
    ...
    public function renderRow()
    {
        ...
        // header_image
        $this->header_image->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!Config("CREATE_UPLOAD_FILE_ON_COPY")) {
                $this->header_image->Upload->DbValue = null;
            }
            if ($this->isShow() || $this->isCopy()) {
                RenderUploadField($this->header_image);
            }
            ...
            if (!Config("CREATE_UPLOAD_FILE_ON_COPY")) {
                $this->footer_image->Upload->DbValue = null;
            }
            if ($this->isShow() || $this->isCopy()) {
                RenderUploadField($this->footer_image);
            }
    ...
    protected function getAddRow()
    {
        ...
        // header_image
        if ($this->header_image->Visible && !$this->header_image->Upload->KeepFile) {
        ...
        // footer_image
        if ($this->footer_image->Visible && !$this->footer_image->Upload->KeepFile) {
    ...
    protected function restoreAddFormFromRow($row)
    {
        if (isset($row['header_image'])) { // header_image
            $this->header_image->setFormValue($row['header_image']);
        }
        if (isset($row['footer_image'])) { // footer_image
            $this->footer_image->setFormValue($row['footer_image']);
        }
    }
