...
/**
 * Table class for jdh_rooms
 */
class JdhRooms extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->room_id->IsAutoIncrement = true; // Autoincrement field
        $this->room_id->IsPrimaryKey = true; // Primary key field
        $this->room_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->room_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        $this->ward_id->Lookup = new Lookup('ward_id', 'jdh_wards', false, 'ward_id', ["ward_name","","",""], '', '', [], [], [], [], [], [], '', '', "`ward_name`");
        ...
        // room_number $tbl, $fldvar, $fldname, $fldexp, $fldbsexp, $fldtype, $fldsize, $flddtfmt, $upload, $fldvirtualexp, $fldvirtual, $forceselect, $fldvirtualsrch, $fldviewtag = "", $fldhtmltag
        $this->room_number = new DbField(
            $this, // Table
            'x_room_number', // Variable name
            'room_number', // Name
            '`room_number`', // Expression
            '`room_number`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`room_number`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->room_number->InputTextType = "text";
        $this->room_number->Nullable = false; // NOT NULL field
        $this->room_number->Required = true; // Required field
        $this->room_number->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->room_number->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`jdh_rooms`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->room_id->setDbValue($conn->lastInsertId());
            $rs['room_id'] = $this->room_id->DbValue;
        }
        return $success;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['room_id']) && !EmptyValue($this->room_id->CurrentValue)) {
                $rs['room_id'] = $this->room_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('room_id', $rs)) {
                AddFilter($where, QuotedName('room_id', $this->Dbid) . '=' . QuotedValue($rs['room_id'], $this->room_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->room_id->CurrentValue : $this->room_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->room_id->CurrentValue = $keys[0];
            } else {
                $this->room_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('room_id', $row) ? $row['room_id'] : null;
        } else {
            $val = !EmptyValue($this->room_id->OldValue) && !$current ? $this->room_id->OldValue : $this->room_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@room_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("jdhroomslist");
    }
    ...
    {
        global $Language;
        if ($pageName == "jdhroomsview") {
            return $Language->phrase("View");
        } elseif ($pageName == "jdhroomsedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "jdhroomsadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jdhroomsview", $parm);
        } else {
            $url = $this->keyUrl("jdhroomsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhroomsedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhroomslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhroomsadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhroomslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jdhroomsdelete");
        }
    }
    ...
    {
        $json = "";
        $json .= "\"room_id\":" . JsonEncode($this->room_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->room_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->room_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("room_id") ?? Route("room_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->room_id->CurrentValue = $key;
            } else {
                $this->room_id->OldValue = $key;
            }
