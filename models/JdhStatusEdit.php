 ...
 * Page class
 */
class JdhStatusEdit extends JdhStatus
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhStatusEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "jdhstatusedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("status_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->status_id->setQueryStringValue($keyValue);
                $this->status_id->setOldValue($this->status_id->QueryStringValue);
            } elseif (Post("status_id") !== null) {
                $this->status_id->setFormValue(Post("status_id"));
                $this->status_id->setOldValue($this->status_id->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("jdhstatuslist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "jdhstatuslist") {
                ...
                if ($this->editRow()) {
                    ...
                    if ($this->UseAjaxActions && GetPageName($returnUrl) != "jdhstatuslist") {
                        Container("flash")->addMessage("Return-Url", $returnUrl); // Save return URL
                        $returnUrl = "jdhstatuslist"; // Return list page content
                    }
