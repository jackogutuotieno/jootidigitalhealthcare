 ...
 * Page class
 */
class JdhPatientVisitsList extends JdhPatientVisits
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhPatientVisitsList";
    ...
    public $FormName = "fjdh_patient_visitslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "jdhpatientvisitslist";
    ...
    public function __construct()
    {
        ...
        // Table object (jdh_patient_visits)
        if (!isset($GLOBALS["jdh_patient_visits"]) || get_class($GLOBALS["jdh_patient_visits"]) == PROJECT_NAMESPACE . "jdh_patient_visits") {
            $GLOBALS["jdh_patient_visits"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "jdhpatientvisitsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "jdhpatientvisitsdelete";
        $this->MultiUpdateUrl = "jdhpatientvisitsupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'jdh_patient_visits');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page => View page
                    ...
                    $result["view"] = $pageName == "jdhpatientvisitsview"; // If View page, no primary button
                } else { // List page
    ...
    {
        if (!SameString($this->visit_id->OldValue, $this->visit_id->CurrentValue)) {
            return false;
        }
        return true;
    }
        ...
        if ($this->isInlineEditRow()) { // Inline-Edit
            ...
            $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
                ...
                if ($this->UseAjaxActions) {
                    ...
                    $opt->Body = <<<INLINEEDIT
                    <div{$divClass}>
                        <button class="ew-grid-link ew-inline-update" title="{$updateTitle}" data-caption="{$updateTitle}" form="fjdh_patient_visitslist" formaction="{$inlineUpdateUrl}">{$updateCaption}</button>
            ...
            $opt->Body .= "<input type=\"hidden\" name=\"" . $this->OldKeyName . "\" id=\"" . $this->OldKeyName . "\" value=\"" . HtmlEncode($this->visit_id->CurrentValue) . "\">";
            return;
        }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fjdh_patient_visitslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fjdh_patient_visitslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"jdh_patient_visits\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("doctor_id", $this->createColumnOption("doctor_id"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fjdh_patient_visitssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fjdh_patient_visitssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function setupRow()
    {
        ...
        if (($this->isGridAdd() || $this->isGridEdit())) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_jdh_patient_visits", "data-rowtype" => ROWTYPE_ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_jdh_patient_visits",
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->visit_type_id->ViewValue === null) { // Lookup from database
                    $filterWrk = SearchFilter("`visit_type_id`", "=", $curVal, DATATYPE_NUMBER, "");
    ...
    protected function editRow()
    {
        ...
        // visit_date
        $this->visit_date->setDbValueDef($rsnew, UnFormatDateTime($this->visit_date->CurrentValue, $this->visit_date->formatPattern()), null, $this->visit_date->ReadOnly);
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['visit_type_id']); // visit_type_id
        $hash .= GetFieldHash($row['doctor_id']); // doctor_id
        $hash .= GetFieldHash($row['visit_date']); // visit_date
        return md5($hash);
    }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fjdh_patient_visitslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
