...
class JdhAppointments extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->appointment_all_day->Lookup = new Lookup($this->appointment_all_day, 'jdh_appointments', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "jdhappointmentsview" => $Language->phrase("View"),
            "jdhappointmentsedit" => $Language->phrase("Edit"),
            "jdhappointmentsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "JdhAppointmentsView",
            Config("API_ADD_ACTION") => "JdhAppointmentsAdd",
            Config("API_EDIT_ACTION") => "JdhAppointmentsEdit",
            Config("API_DELETE_ACTION") => "JdhAppointmentsDelete",
            Config("API_LIST_ACTION") => "JdhAppointmentsList",
            default => ""
        };
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jdhappointmentsdelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'jdh_appointments');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'jdh_appointments', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'jdh_appointments', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'jdh_appointments', $fldname, $key, $oldvalue);
            }
        }
    }
