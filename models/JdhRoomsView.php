 ...
 * Page class
 */
class JdhRoomsView extends JdhRooms
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhRoomsView";
    ...
    // CSS class/style
    public $CurrentPageName = "jdhroomsview";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("room_id") ?? Route("room_id")) !== null) {
            ...
            $this->RecKey["room_id"] = $this->room_id->QueryStringValue;
        } elseif (Post("room_id") !== null) {
            $this->room_id->setFormValue(Post("room_id"));
            $this->RecKey["room_id"] = $this->room_id->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->room_id->setQueryStringValue($keyValue);
            $this->RecKey["room_id"] = $this->room_id->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "jdhroomslist"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "jdhroomslist"; // No matching record, return to list
                    }
                break;
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fjdh_roomsview" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-key="' . HtmlEncode(ArrayToJsonAttribute($this->RecKey)) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
