 ...
 * Page class
 */
class JdhPatientCasesView extends JdhPatientCases
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhPatientCasesView";
    ...
    // CSS class/style
    public $CurrentPageName = "jdhpatientcasesview";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("case_id") ?? Route("case_id")) !== null) {
            ...
            $this->RecKey["case_id"] = $this->case_id->QueryStringValue;
        } elseif (Post("case_id") !== null) {
            $this->case_id->setFormValue(Post("case_id"));
            $this->RecKey["case_id"] = $this->case_id->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->case_id->setQueryStringValue($keyValue);
            $this->RecKey["case_id"] = $this->case_id->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "jdhpatientcaseslist"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "jdhpatientcaseslist"; // No matching record, return to list
                    }
                break;
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fjdh_patient_casesview" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-key="' . HtmlEncode(ArrayToJsonAttribute($this->RecKey)) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
