...
/**
 * Table class for jdh_services
 */
class JdhServices extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->category_id->Lookup = new Lookup($this->category_id, 'jdh_service_category', false, 'category_id', ["category_name","","",""], '', '', [], ["x_subcategory_id"], [], [], [], [], false, '', '', "`category_name`");
        ...
        $this->subcategory_id->Lookup = new Lookup($this->subcategory_id, 'jdh_service_subcategory', false, 'subcategory_id', ["subcategory_name","","",""], '', '', ["x_category_id"], [], ["category_id"], ["x_category_id"], [], [], false, '', '', "`subcategory_name`");
        ...
        // service_description
        $this->service_description = new DbField(
            $this, // Table
            'x_service_description', // Variable name
            'service_description', // Name
            '`service_description`', // Expression
            '`service_description`', // Basic search expression
            200, // Type
            65535, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`service_description`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->service_description->InputTextType = "text";
        $this->service_description->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['service_description'] = &$this->service_description;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "jdh_services";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->service_id->setDbValue($conn->lastInsertId());
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['service_id']) && !EmptyValue($this->service_id->CurrentValue)) {
                $rs['service_id'] = $this->service_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->service_description->DbValue = $row['service_description'];
    ...
    {
        $keys = [];
        $val = $current ? $this->service_id->CurrentValue : $this->service_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->service_id->CurrentValue = $keys[0];
            } else {
                $this->service_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            ...
            $val = !EmptyValue($this->service_id->OldValue) && !$current ? $this->service_id->OldValue : $this->service_id->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("jdhserviceslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "jdhservicesview" => $Language->phrase("View"),
            "jdhservicesedit" => $Language->phrase("Edit"),
            "jdhservicesadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "JdhServicesView",
            Config("API_ADD_ACTION") => "JdhServicesAdd",
            Config("API_EDIT_ACTION") => "JdhServicesEdit",
            Config("API_DELETE_ACTION") => "JdhServicesDelete",
            Config("API_LIST_ACTION") => "JdhServicesList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jdhservicesview", $parm);
        } else {
            $url = $this->keyUrl("jdhservicesview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhservicesedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhserviceslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhservicesadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhserviceslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jdhservicesdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"service_id\":" . VarToJson($this->service_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->service_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->service_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("service_id") ?? Route("service_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function renderListRow()
    {
        ...
        $this->service_description->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        $this->service_description->EditValue = $this->service_description->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->service_description);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->service_description);
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM jdh_services";
