 ...
 * Page class
 */
class JdhTestReportsGrid extends JdhTestReports
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhTestReportsGrid";
    ...
    public $FormName = "fjdh_test_reportsgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "jdhtestreportsgrid";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["jdh_test_reports"]) || $GLOBALS["jdh_test_reports"]::class == PROJECT_NAMESPACE . "jdh_test_reports") {
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->report_id->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_request_id") && $CurrentForm->hasValue("o_request_id") && $this->request_id->CurrentValue != $this->request_id->DefaultValue) {
            return false;
        }
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_jdh_test_reports", "data-rowtype" => RowType::ADD]);
    ...
    {
        $this->report_attachment->Upload->Index = $this->RowIndex;
        $this->report_submittedby_user_id->DefaultValue = CurrentUserID();
        $this->report_submittedby_user_id->OldValue = $this->report_submittedby_user_id->DefaultValue;
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->report_id->CurrentValue = $this->report_id->FormValue;
        }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->report_id->Visible && $this->report_id->Required) {
            ...
            if ($this->report_findings->Visible && $this->report_findings->Required) {
            ...
            if ($this->report_date->Visible && $this->report_date->Required) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // report_date
        $this->report_date->setDbValueDef($rsnew, UnFormatDateTime($this->report_date->CurrentValue, $this->report_date->formatPattern()), $this->report_date->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['report_findings'])) { // report_findings
            $this->report_findings->CurrentValue = $row['report_findings'];
        }
        if (isset($row['report_date'])) { // report_date
            $this->report_date->CurrentValue = $row['report_date'];
        }
    }
    ...
    protected function getAddRow()
    {
        ...
        // report_date
        $this->report_date->setDbValueDef($rsnew, UnFormatDateTime($this->report_date->CurrentValue, $this->report_date->formatPattern()), false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['report_findings'])) { // report_findings
            $this->report_findings->setFormValue($row['report_findings']);
        }
        if (isset($row['report_date'])) { // report_date
            $this->report_date->setFormValue($row['report_date']);
        }
        if (isset($row['report_submittedby_user_id'])) { // report_submittedby_user_id
            $this->report_submittedby_user_id->setFormValue($row['report_submittedby_user_id']);
        }
    }
