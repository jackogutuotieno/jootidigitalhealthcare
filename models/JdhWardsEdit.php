 ...
 * Page class
 */
class JdhWardsEdit extends JdhWards
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhWardsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "jdhwardsedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("ward_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->ward_id->setQueryStringValue($keyValue);
                $this->ward_id->setOldValue($this->ward_id->QueryStringValue);
            } elseif (Post("ward_id") !== null) {
                $this->ward_id->setFormValue(Post("ward_id"));
                $this->ward_id->setOldValue($this->ward_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("ward_id") ?? Route("ward_id")) !== null) {
                    $this->ward_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->ward_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("jdhwardslist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'ward_name' first before field var 'x_ward_name'
        $val = $CurrentForm->hasValue("ward_name") ? $CurrentForm->getValue("ward_name") : $CurrentForm->getValue("x_ward_name");
        if (!$this->ward_name->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ward_name->Visible = false; // Disable update for API request
            } else {
                $this->ward_name->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->ward_name->CurrentValue = $this->ward_name->FormValue;
    ...
    public function renderRow()
    {
        ...
        // ward_name
        $this->ward_name->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->ward_name->EditValue = HtmlEncode($this->ward_name->CurrentValue);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("jdhwardslist"), "", $this->TableVar, true);
        $pageId = "edit";
