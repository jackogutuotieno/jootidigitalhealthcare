 ...
 * Page class
 */
class JdhInvoiceAdd extends JdhInvoice
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhInvoiceAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "jdhinvoiceadd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "jdhinvoiceview") {
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->invoice_title->Visible && $this->invoice_title->Required) {
            ...
            if ($this->invoice_description->Visible && $this->invoice_description->Required) {
        ...
        if (in_array("jdh_invoice_items", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("JdhInvoiceItemsGrid");
            if (in_array("jdh_invoice_items", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->invoice_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "jdh_invoice_items"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->invoice_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // invoice_description
        $this->invoice_description->setDbValueDef($rsnew, $this->invoice_description->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['invoice_title'])) { // invoice_title
            $this->invoice_title->setFormValue($row['invoice_title']);
        }
        if (isset($row['invoice_description'])) { // invoice_description
            $this->invoice_description->setFormValue($row['invoice_description']);
        }
    }
