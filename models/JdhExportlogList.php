 ...
 * Page class
 */
class JdhExportlogList extends JdhExportlog
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhExportlogList";
    ...
    public $FormName = "fjdh_exportloglist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "jdhexportloglist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "jdhexportlogadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "jdhexportlogdelete";
        $this->MultiUpdateUrl = "jdhexportlogupdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fjdh_exportloggrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fjdh_exportlogsrch");
        }
        $filterList = Concat($filterList, $this->FileId->AdvancedSearch->toJson(), ","); // Field FileId
        ...
        $filterList = Concat($filterList, $this->_ExportType->AdvancedSearch->toJson(), ","); // Field ExportType
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fjdh_exportlogsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field FileId
        $this->FileId->AdvancedSearch->SearchValue = @$filter["x_FileId"];
        $this->FileId->AdvancedSearch->SearchOperator = @$filter["z_FileId"];
        $this->FileId->AdvancedSearch->SearchCondition = @$filter["v_FileId"];
        $this->FileId->AdvancedSearch->SearchValue2 = @$filter["y_FileId"];
        $this->FileId->AdvancedSearch->SearchOperator2 = @$filter["w_FileId"];
        $this->FileId->AdvancedSearch->save();
        ...
        // Field ExportType
        $this->_ExportType->AdvancedSearch->SearchValue = @$filter["x__ExportType"];
        $this->_ExportType->AdvancedSearch->SearchOperator = @$filter["z__ExportType"];
        $this->_ExportType->AdvancedSearch->SearchCondition = @$filter["v__ExportType"];
        $this->_ExportType->AdvancedSearch->SearchValue2 = @$filter["y__ExportType"];
        $this->_ExportType->AdvancedSearch->SearchOperator2 = @$filter["w__ExportType"];
        $this->_ExportType->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->FileId;
        $searchFlds[] = &$this->User;
        $searchFlds[] = &$this->_ExportType;
        $searchFlds[] = &$this->_Table;
        $searchFlds[] = &$this->KeyValue;
        $searchFlds[] = &$this->Filename;
        $searchFlds[] = &$this->__Request;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->FileId); // FileId
            $this->updateSort($this->DateTime); // DateTime
            $this->updateSort($this->User); // User
            $this->updateSort($this->_ExportType); // ExportType
            $this->updateSort($this->_Table); // Table
            $this->updateSort($this->KeyValue); // KeyValue
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fjdh_exportloglist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fjdh_exportloglist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->FileId->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"jdh_exportlog\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("FileId", $this->createColumnOption("FileId"));
            $option->add("DateTime", $this->createColumnOption("DateTime"));
            $option->add("User", $this->createColumnOption("User"));
            $option->add("ExportType", $this->createColumnOption("ExportType"));
            $option->add("Table", $this->createColumnOption("Table"));
            $option->add("KeyValue", $this->createColumnOption("KeyValue"));
            $option->add("Filename", $this->createColumnOption("Filename"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fjdh_exportlogsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fjdh_exportlogsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fjdh_exportloglist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if (($this->isGridAdd() || $this->isGridEdit())) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_jdh_exportlog", "data-rowtype" => ROWTYPE_ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_jdh_exportlog",
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fjdh_exportlogsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
