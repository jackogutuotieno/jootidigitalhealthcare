...
class BedsEdit extends Beds
{
    ...
    // CSS class/style
    public $CurrentPageName = "bedsedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("bed_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->bed_id->setQueryStringValue($keyValue);
                $this->bed_id->setOldValue($this->bed_id->QueryStringValue);
            } elseif (Post("bed_id") !== null) {
                $this->bed_id->setFormValue(Post("bed_id"));
                $this->bed_id->setOldValue($this->bed_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("bed_id") ?? Route("bed_id")) !== null) {
                    $this->bed_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            ...
            case "update": // Update
                ...
                if ($this->editRow()) {
                    ...
                    if ($this->UseAjaxActions && GetPageName($returnUrl) != "bedslist") {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->room_id->ViewValue = $this->room_id->Lookup !== null && is_array($this->room_id->lookupOptions()) ? $curVal : null;
            }
            if ($this->room_id->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter("`room_id`", "=", $this->room_id->CurrentValue, DATATYPE_NUMBER, "");
                }
                ...
                foreach ($arwrk as &$row) {
                    $row = $this->room_id->Lookup->renderViewRow($row);
                }
    ...
    protected function editRow()
    {
        ...
        // ward_id
        $this->ward_id->setDbValueDef($rsnew, $this->ward_id->CurrentValue, 0, $this->ward_id->ReadOnly);

        // room_id
        $this->room_id->setDbValueDef($rsnew, $this->room_id->CurrentValue, 0, $this->room_id->ReadOnly);

        // bed_number
        $this->bed_number->setDbValueDef($rsnew, $this->bed_number->CurrentValue, 0, $this->bed_number->ReadOnly);
