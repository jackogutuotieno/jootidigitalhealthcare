 ...
 * Page class
 */
class JdhInvoiceView extends JdhInvoice
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhInvoiceView";
    ...
    // CSS class/style
    public $CurrentPageName = "jdhinvoiceview";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "jdhinvoicelist"; // No matching record, return to list
                    }
                break;
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fjdh_invoiceview" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-key="' . ArrayToJsonAttribute($this->RecKey) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("jdh_invoice_items", explode(",", $this->getCurrentDetailTable()))) {
            ...
            $rsdetail = $jdh_invoice_items->loadRs($jdh_invoice_items->getDetailFilterFromSession(), $jdh_invoice_items->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($jdh_invoice_items);
                    $jdh_invoice_items->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
