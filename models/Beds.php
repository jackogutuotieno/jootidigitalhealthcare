...
/**
 * Table class for beds
 */
class Beds extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->bed_id->IsAutoIncrement = true; // Autoincrement field
        $this->bed_id->IsPrimaryKey = true; // Primary key field
        $this->bed_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->bed_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        $this->ward_id->Lookup = new Lookup('ward_id', 'jdh_wards', false, 'ward_id', ["ward_name","","",""], '', '', [], ["x_room_id"], [], [], [], [], '', '', "`ward_name`");
        ...
        // room_id $tbl, $fldvar, $fldname, $fldexp, $fldbsexp, $fldtype, $fldsize, $flddtfmt, $upload, $fldvirtualexp, $fldvirtual, $forceselect, $fldvirtualsrch, $fldviewtag = "", $fldhtmltag
        $this->room_id = new DbField(
            $this, // Table
            'x_room_id', // Variable name
            'room_id', // Name
            '`room_id`', // Expression
            '`room_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`room_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->room_id->InputTextType = "text";
        $this->room_id->Nullable = false; // NOT NULL field
        $this->room_id->Required = true; // Required field
        $this->room_id->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->room_id->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->room_id->Lookup = new Lookup('room_id', 'jdh_rooms', false, 'room_id', ["room_number","","",""], '', '', ["x_ward_id"], [], ["ward_id"], ["x_ward_id"], [], [], '', '', "`room_number`");
        $this->room_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->room_id->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['room_id'] = &$this->room_id;

        // bed_number $tbl, $fldvar, $fldname, $fldexp, $fldbsexp, $fldtype, $fldsize, $flddtfmt, $upload, $fldvirtualexp, $fldvirtual, $forceselect, $fldvirtualsrch, $fldviewtag = "", $fldhtmltag
        $this->bed_number = new DbField(
            $this, // Table
            'x_bed_number', // Variable name
            'bed_number', // Name
            '`bed_number`', // Expression
            '`bed_number`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`bed_number`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->bed_number->InputTextType = "text";
        $this->bed_number->Nullable = false; // NOT NULL field
        $this->bed_number->Required = true; // Required field
        $this->bed_number->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->bed_number->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['bed_number'] = &$this->bed_number;
        ...
        $this->status_id->Lookup = new Lookup('status_id', 'beds', false, '', ["","","",""], '', '', [], [], [], [], [], [], '', '', "");
        $this->status_id->OptionCount = 2;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`beds`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->bed_id->setDbValue($conn->lastInsertId());
            $rs['bed_id'] = $this->bed_id->DbValue;
        }
        return $success;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['bed_id']) && !EmptyValue($this->bed_id->CurrentValue)) {
                $rs['bed_id'] = $this->bed_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('bed_id', $rs)) {
                AddFilter($where, QuotedName('bed_id', $this->Dbid) . '=' . QuotedValue($rs['bed_id'], $this->bed_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->bed_number->DbValue = $row['bed_number'];
    ...
    {
        $keys = [];
        $val = $current ? $this->bed_id->CurrentValue : $this->bed_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->bed_id->CurrentValue = $keys[0];
            } else {
                $this->bed_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('bed_id', $row) ? $row['bed_id'] : null;
        } else {
            $val = !EmptyValue($this->bed_id->OldValue) && !$current ? $this->bed_id->OldValue : $this->bed_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@bed_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("bedslist");
    }
    ...
    {
        global $Language;
        if ($pageName == "bedsview") {
            return $Language->phrase("View");
        } elseif ($pageName == "bedsedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "bedsadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("bedsview", $parm);
        } else {
            $url = $this->keyUrl("bedsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("bedslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("bedslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $json = "";
        $json .= "\"bed_id\":" . JsonEncode($this->bed_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->bed_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->bed_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("bed_id") ?? Route("bed_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->bed_id->CurrentValue = $key;
            } else {
                $this->bed_id->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->bed_number);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->bed_number);
