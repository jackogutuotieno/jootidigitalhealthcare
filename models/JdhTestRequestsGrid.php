 ...
 * Page class
 */
class JdhTestRequestsGrid extends JdhTestRequests
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhTestRequestsGrid";
    ...
    public $FormName = "fjdh_test_requestsgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "jdhtestrequestsgrid";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["jdh_test_requests"]) || $GLOBALS["jdh_test_requests"]::class == PROJECT_NAMESPACE . "jdh_test_requests") {
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fjdh_test_requestsgrid";
        }
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->request_id->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_request_title") && $CurrentForm->hasValue("o_request_title") && $this->request_title->CurrentValue != $this->request_title->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_request_service_id") && $CurrentForm->hasValue("o_request_service_id") && $this->request_service_id->CurrentValue != $this->request_service_id->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_request_description") && $CurrentForm->hasValue("o_request_description") && $this->request_description->CurrentValue != $this->request_description->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_request_date") && $CurrentForm->hasValue("o_request_date") && $this->request_date->CurrentValue != $this->request_date->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_status_id") && $CurrentForm->hasValue("o_status_id") && $this->status_id->CurrentValue != $this->status_id->DefaultValue) {
            return false;
        }
        return true;
    }
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView() && $this->showOptionLink("view")) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"jdh_test_requests\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit() && $this->showOptionLink("edit")) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"jdh_test_requests\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->CurrentMode == "view") { // Check view mode
            ...
            if ($this->ModalAdd && !IsMobile()) {
                $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"jdh_test_requests\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
            } else {
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_jdh_test_requests", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_jdh_test_requests",
    ...
    {
        $this->requested_by_user_id->DefaultValue = CurrentUserID();
        $this->requested_by_user_id->OldValue = $this->requested_by_user_id->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_request_title")) {
            $this->request_title->setOldValue($CurrentForm->getValue("o_request_title"));
        }
        ...
        if ($CurrentForm->hasValue("o_request_service_id")) {
            $this->request_service_id->setOldValue($CurrentForm->getValue("o_request_service_id"));
        }
        ...
        if ($CurrentForm->hasValue("o_request_description")) {
            $this->request_description->setOldValue($CurrentForm->getValue("o_request_description"));
        }
        ...
        if (!$this->request_date->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->request_date->setFormValue($val);
            }
            $this->request_date->CurrentValue = UnFormatDateTime($this->request_date->CurrentValue, $this->request_date->formatPattern());
        }
        if ($CurrentForm->hasValue("o_request_date")) {
            $this->request_date->setOldValue($CurrentForm->getValue("o_request_date"));
        }
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->request_service_id->ViewValue === null) { // Lookup from database
                    $filterWrk = SearchFilter($this->request_service_id->Lookup->getTable()->Fields["service_id"]->searchExpression(), "=", $curVal, $this->request_service_id->Lookup->getTable()->Fields["service_id"]->searchDataType(), "");
            ...
            if ($curVal != "") {
                ...
                $this->request_service_id->ViewValue = $this->request_service_id->Lookup !== null && is_array($this->request_service_id->lookupOptions()) && count($this->request_service_id->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->request_service_id->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->request_service_id->Lookup->getTable()->Fields["service_id"]->searchExpression(), "=", $this->request_service_id->CurrentValue, $this->request_service_id->Lookup->getTable()->Fields["service_id"]->searchDataType(), "");
                }
                $lookupFilter = $this->request_service_id->getSelectFilter($this); // PHP
            ...
            // request_date
            $this->request_date->setupEditAttributes();
            $this->request_date->EditValue = $this->request_date->CurrentValue;
            $this->request_date->EditValue = FormatDateTime($this->request_date->EditValue, $this->request_date->formatPattern());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->request_title->Visible && $this->request_title->Required) {
            ...
            if ($this->request_service_id->Visible && $this->request_service_id->Required) {
            ...
            if ($this->request_description->Visible && $this->request_description->Required) {
            ...
            if ($this->request_date->Visible && $this->request_date->Required) {
    ...
    protected function getAddRow()
    {
        ...
        // request_service_id
        $this->request_service_id->setDbValueDef($rsnew, $this->request_service_id->CurrentValue, false);

        // request_description
        $this->request_description->setDbValueDef($rsnew, $this->request_description->CurrentValue, false);

        // request_date
        $this->request_date->setDbValueDef($rsnew, UnFormatDateTime($this->request_date->CurrentValue, $this->request_date->formatPattern()), false);
        ...
        if (!$Security->isAdmin() && $Security->isLoggedIn()) { // Non system admin
            $rsnew['requested_by_user_id'] = CurrentUserID();
        }
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['request_title'])) { // request_title
            $this->request_title->setFormValue($row['request_title']);
        }
        if (isset($row['request_service_id'])) { // request_service_id
            $this->request_service_id->setFormValue($row['request_service_id']);
        }
        if (isset($row['request_description'])) { // request_description
            $this->request_description->setFormValue($row['request_description']);
        }
        if (isset($row['request_date'])) { // request_date
            $this->request_date->setFormValue($row['request_date']);
        }
        if (isset($row['status_id'])) { // status_id
            $this->status_id->setFormValue($row['status_id']);
        }
        if (isset($row['requested_by_user_id'])) { // requested_by_user_id
            $this->requested_by_user_id->setFormValue($row['requested_by_user_id']);
        }
    }
