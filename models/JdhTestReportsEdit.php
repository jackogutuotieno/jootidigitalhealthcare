 ...
 * Page class
 */
class JdhTestReportsEdit extends JdhTestReports
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhTestReportsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("report_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->report_id->setQueryStringValue($keyValue);
                $this->report_id->setOldValue($this->report_id->QueryStringValue);
            } elseif (Post("report_id") !== null) {
                $this->report_id->setFormValue(Post("report_id"));
                $this->report_id->setOldValue($this->report_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("report_id") ?? Route("report_id")) !== null) {
                    ...
                    $this->report_id->CurrentValue = null;
                }
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->report_attachment->Visible && $this->report_attachment->Required) {
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['report_attachment'])) { // report_attachment
            $this->report_attachment->CurrentValue = $row['report_attachment'];
        }
    }
