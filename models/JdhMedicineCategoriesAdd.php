 ...
 * Page class
 */
class JdhMedicineCategoriesAdd extends JdhMedicineCategories
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "JdhMedicineCategoriesAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "jdhmedicinecategoriesadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("jdhmedicinecategorieslist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "jdhmedicinecategorieslist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "jdhmedicinecategoriesview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "jdhmedicinecategorieslist") {
                            ...
                            $returnUrl = "jdhmedicinecategorieslist"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'category_description' first before field var 'x_category_description'
        $val = $CurrentForm->hasValue("category_description") ? $CurrentForm->getValue("category_description") : $CurrentForm->getValue("x_category_description");
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->category_description->EditValue = HtmlEncode($this->category_description->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->category_name->Visible && $this->category_name->Required) {
            ...
            if ($this->category_description->Visible && $this->category_description->Required) {
                if (!$this->category_description->IsDetailKey && EmptyValue($this->category_description->FormValue)) {
    ...
    {
        if (isset($row['category_name'])) { // category_name
            $this->category_name->setFormValue($row['category_name']);
        }
        if (isset($row['category_description'])) { // category_description
            $this->category_description->setFormValue($row['category_description']);
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("jdhmedicinecategorieslist"), "", $this->TableVar, true);
