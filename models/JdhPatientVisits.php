...
/**
 * Table class for jdh_patient_visits
 */
class JdhPatientVisits extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = "jdh_patient_visits";
        $this->TableName = 'jdh_patient_visits';
        $this->TableType = "TABLE";
        $this->ImportUseTransaction = $this->supportsTransaction() && Config("IMPORT_USE_TRANSACTION");
        $this->UseTransaction = $this->supportsTransaction() && Config("USE_TRANSACTION");

        // Update Table
        $this->UpdateTable = "jdh_patient_visits";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->visit_type_id->Lookup = new Lookup($this->visit_type_id, 'jdh_visit_types', false, 'visit_type_id', ["visit_type","","",""], '', '', [], [], [], [], [], [], false, '', '', "`visit_type`");
        ...
        $this->user_id->Lookup = new Lookup($this->user_id, 'jdh_users', false, 'user_id', ["user_id","first_name","last_name",""], '', '', [], [], [], [], [], [], false, '', '', "CONCAT(COALESCE(`user_id`, ''),'" . ValueSeparator(1, $this->user_id) . "',COALESCE(`first_name`,''),'" . ValueSeparator(2, $this->user_id) . "',COALESCE(`last_name`,''))");
        ...
        $this->insurance_id->Raw = true;
        $this->insurance_id->setSelectMultiple(false); // Select one
        ...
        $this->insurance_id->Lookup = new Lookup($this->insurance_id, 'jdh_insurance', false, 'insurance_id', ["insurance_name","","",""], '', '', [], [], [], [], [], [], false, '', '', "`insurance_name`");
        ...
        $this->visit_description = new DbField(
            $this, // Table
            'x_visit_description', // Variable name
            'visit_description', // Name
            '`visit_description`', // Expression
            '`visit_description`', // Basic search expression
            200, // Type
            65535, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`visit_description`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->visit_description->InputTextType = "text";
        $this->visit_description->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['visit_description'] = &$this->visit_description;

        // visit_date
        $this->visit_date = new DbField(
            $this, // Table
            'x_visit_date', // Variable name
            'visit_date', // Name
            '`visit_date`', // Expression
            CastDateFieldForLike("`visit_date`", 11, "DB"), // Basic search expression
            135, // Type
            19, // Size
            11, // Date/Time format
            false, // Is upload field
            '`visit_date`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->visit_date->InputTextType = "text";
        $this->visit_date->Raw = true;
        $this->visit_date->Nullable = false; // NOT NULL field
        $this->visit_date->Required = true; // Required field
        $this->visit_date->DefaultErrorMessage = str_replace("%s", DateFormat(11), $Language->phrase("IncorrectDate"));
        $this->visit_date->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "jdh_patient_visits";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("jdhpatientvisitslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "jdhpatientvisitsview" => $Language->phrase("View"),
            "jdhpatientvisitsedit" => $Language->phrase("Edit"),
            "jdhpatientvisitsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "JdhPatientVisitsView",
            Config("API_ADD_ACTION") => "JdhPatientVisitsAdd",
            Config("API_EDIT_ACTION") => "JdhPatientVisitsEdit",
            Config("API_DELETE_ACTION") => "JdhPatientVisitsDelete",
            Config("API_LIST_ACTION") => "JdhPatientVisitsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jdhpatientvisitsview", $parm);
        } else {
            $url = $this->keyUrl("jdhpatientvisitsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "jdhpatientvisitsadd?" . $parm;
        } else {
            $url = "jdhpatientvisitsadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientvisitsedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientvisitslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientvisitsadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientvisitslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jdhpatientvisitsdelete", $parm);
        }
    }
    ...
    public function renderListRow()
    {
        ...
        if ($curVal != "") {
            ...
            if ($this->visit_type_id->ViewValue === null) { // Lookup from database
                $filterWrk = SearchFilter($this->visit_type_id->Lookup->getTable()->Fields["visit_type_id"]->searchExpression(), "=", $curVal, $this->visit_type_id->Lookup->getTable()->Fields["visit_type_id"]->searchDataType(), "");
        ...
        if ($curVal != "") {
            ...
            if ($this->insurance_id->ViewValue === null) { // Lookup from database
                $filterWrk = SearchFilter($this->insurance_id->Lookup->getTable()->Fields["insurance_id"]->searchExpression(), "=", $curVal, $this->insurance_id->Lookup->getTable()->Fields["insurance_id"]->searchDataType(), "");
        ...
        $this->visit_description->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        $this->visit_description->PlaceHolder = RemoveHtml($this->visit_description->caption());

        // visit_date
        $this->visit_date->setupEditAttributes();
        $this->visit_date->EditValue = FormatDateTime($this->visit_date->CurrentValue, $this->visit_date->formatPattern());
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM jdh_patient_visits";
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'jdh_patient_visits');
    }
