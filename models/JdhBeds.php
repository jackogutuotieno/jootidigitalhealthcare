...
/**
 * Table class for jdh_beds
 */
class JdhBeds extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->facility_id->Lookup = new Lookup($this->facility_id, 'jdh_facility_units', false, 'id', ["unit_name","","",""], '', '', [], ["x_ward_id"], [], [], [], [], false, '', '', "`unit_name`");
        ...
        $this->ward_id->Lookup = new Lookup($this->ward_id, 'jdh_wards', false, 'ward_id', ["ward_name","","",""], '', '', ["x_facility_id"], [], ["facility_id"], ["x_facility_id"], [], [], false, '', '', "`ward_name`");
        ...
        // assigned
        $this->assigned = new DbField(
            $this, // Table
            'x_assigned', // Variable name
            'assigned', // Name
            '`assigned`', // Expression
            '`assigned`', // Basic search expression
            16, // Type
            1, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`assigned`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->assigned->InputTextType = "text";
        $this->assigned->Raw = true;
        $this->assigned->Nullable = false; // NOT NULL field
        $this->assigned->setDataType(DataType::BOOLEAN);
        $this->assigned->Lookup = new Lookup($this->assigned, 'jdh_beds', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->assigned->OptionCount = 2;
        $this->assigned->DefaultErrorMessage = $Language->phrase("IncorrectField");
        $this->assigned->SearchOperators = ["=", "<>"];
        $this->Fields['assigned'] = &$this->assigned;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "jdh_beds";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->assigned->DbValue = $row['assigned'];
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("jdhbedslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "jdhbedsview" => $Language->phrase("View"),
            "jdhbedsedit" => $Language->phrase("Edit"),
            "jdhbedsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "JdhBedsView",
            Config("API_ADD_ACTION") => "JdhBedsAdd",
            Config("API_EDIT_ACTION") => "JdhBedsEdit",
            Config("API_DELETE_ACTION") => "JdhBedsDelete",
            Config("API_LIST_ACTION") => "JdhBedsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jdhbedsview", $parm);
        } else {
            $url = $this->keyUrl("jdhbedsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhbedsedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhbedslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhbedsadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhbedslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jdhbedsdelete", $parm);
        }
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->assigned);
                } else {
                    $doc->exportCaption($this->id);
                    $doc->exportCaption($this->facility_id);
                    $doc->exportCaption($this->ward_id);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'jdh_beds');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'jdh_beds', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'jdh_beds', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'jdh_beds', $fldname, $key, $oldvalue);
            }
        }
    }
