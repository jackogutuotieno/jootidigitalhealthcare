...
/**
 * Table class for jdh_patient_services
 */
class JdhPatientServices extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = "jdh_patient_services";
        $this->TableName = 'jdh_patient_services';
        $this->TableType = "TABLE";
        $this->ImportUseTransaction = $this->supportsTransaction() && Config("IMPORT_USE_TRANSACTION");
        $this->UseTransaction = $this->supportsTransaction() && Config("USE_TRANSACTION");

        // Update Table
        $this->UpdateTable = "`jdh_patient_services`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // patient_service_id $tbl, $fldvar, $fldname, $fldexp, $fldbsexp, $fldtype, $fldsize, $flddtfmt, $upload, $fldvirtualexp, $fldvirtual, $forceselect, $fldvirtualsrch, $fldviewtag = "", $fldhtmltag
        $this->patient_service_id = new DbField(
            $this, // Table
            'x_patient_service_id', // Variable name
            'patient_service_id', // Name
            '`patient_service_id`', // Expression
            '`patient_service_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`patient_service_id`', // Virtual expression
            false, // Is virtual
        ...
        $this->patient_service_id->InputTextType = "text";
        $this->patient_service_id->IsAutoIncrement = true; // Autoincrement field
        $this->patient_service_id->IsPrimaryKey = true; // Primary key field
        $this->patient_service_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->patient_service_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['patient_service_id'] = &$this->patient_service_id;
        ...
        $this->service_id->Lookup = new Lookup('service_id', 'jdh_services', false, 'service_id', ["service_name","","",""], '', '', [], [], [], [], [], [], '', '', "`service_name`");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`jdh_patient_services`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->patient_service_id->setDbValue($conn->lastInsertId());
            $rs['patient_service_id'] = $this->patient_service_id->DbValue;
        }
        return $success;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['patient_service_id']) && !EmptyValue($this->patient_service_id->CurrentValue)) {
                $rs['patient_service_id'] = $this->patient_service_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('patient_service_id', $rs)) {
                AddFilter($where, QuotedName('patient_service_id', $this->Dbid) . '=' . QuotedValue($rs['patient_service_id'], $this->patient_service_id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->patient_service_id->DbValue = $row['patient_service_id'];
    ...
    {
        return "`patient_service_id` = @patient_service_id@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->patient_service_id->CurrentValue : $this->patient_service_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->patient_service_id->CurrentValue = $keys[0];
            } else {
                $this->patient_service_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('patient_service_id', $row) ? $row['patient_service_id'] : null;
        } else {
            $val = !EmptyValue($this->patient_service_id->OldValue) && !$current ? $this->patient_service_id->OldValue : $this->patient_service_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@patient_service_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("jdhpatientserviceslist");
    }
    ...
    {
        global $Language;
        if ($pageName == "jdhpatientservicesview") {
            return $Language->phrase("View");
        } elseif ($pageName == "jdhpatientservicesedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "jdhpatientservicesadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "JdhPatientServicesView";
            case Config("API_ADD_ACTION"):
                return "JdhPatientServicesAdd";
            case Config("API_EDIT_ACTION"):
                return "JdhPatientServicesEdit";
            case Config("API_DELETE_ACTION"):
                return "JdhPatientServicesDelete";
            case Config("API_LIST_ACTION"):
                return "JdhPatientServicesList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jdhpatientservicesview", $parm);
        } else {
            $url = $this->keyUrl("jdhpatientservicesview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "jdhpatientservicesadd?" . $parm;
        } else {
            $url = "jdhpatientservicesadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientservicesedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientserviceslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientservicesadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientserviceslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jdhpatientservicesdelete");
        }
    }
    ...
    {
        $json = "";
        $json .= "\"patient_service_id\":" . JsonEncode($this->patient_service_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->patient_service_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->patient_service_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("patient_service_id") ?? Route("patient_service_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->patient_service_id->CurrentValue = $key;
            } else {
                $this->patient_service_id->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->patient_service_id->setDbValue($row['patient_service_id']);
    ...
    {
        global $Response;
        $listPage = "JdhPatientServicesList";
    ...
    public function renderListRow()
    {
        ...
        $this->patient_service_id->TooltipValue = "";
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->patient_service_id);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->patient_service_id);
