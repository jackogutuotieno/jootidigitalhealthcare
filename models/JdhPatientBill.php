...
/**
 * Table class for jdh_patient_bill
 */
class JdhPatientBill extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "`jdh_patient_bill`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // bill_id $tbl, $fldvar, $fldname, $fldexp, $fldbsexp, $fldtype, $fldsize, $flddtfmt, $upload, $fldvirtualexp, $fldvirtual, $forceselect, $fldvirtualsrch, $fldviewtag = "", $fldhtmltag
        $this->bill_id = new DbField(
            $this, // Table
            'x_bill_id', // Variable name
            'bill_id', // Name
            '`bill_id`', // Expression
            '`bill_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`bill_id`', // Virtual expression
            false, // Is virtual
        ...
        $this->bill_id->InputTextType = "text";
        $this->bill_id->IsAutoIncrement = true; // Autoincrement field
        $this->bill_id->IsPrimaryKey = true; // Primary key field
        $this->bill_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->bill_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        // bill_description $tbl, $fldvar, $fldname, $fldexp, $fldbsexp, $fldtype, $fldsize, $flddtfmt, $upload, $fldvirtualexp, $fldvirtual, $forceselect, $fldvirtualsrch, $fldviewtag = "", $fldhtmltag
        $this->bill_description = new DbField(
            $this, // Table
            'x_bill_description', // Variable name
            'bill_description', // Name
            '`bill_description`', // Expression
            '`bill_description`', // Basic search expression
            201, // Type
            65535, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`bill_description`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->bill_description->InputTextType = "text";
        $this->bill_description->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['bill_description'] = &$this->bill_description;

        // bill_date $tbl, $fldvar, $fldname, $fldexp, $fldbsexp, $fldtype, $fldsize, $flddtfmt, $upload, $fldvirtualexp, $fldvirtual, $forceselect, $fldvirtualsrch, $fldviewtag = "", $fldhtmltag
        $this->bill_date = new DbField(
            $this, // Table
            'x_bill_date', // Variable name
            'bill_date', // Name
            '`bill_date`', // Expression
            CastDateFieldForLike("`bill_date`", 0, "DB"), // Basic search expression
            135, // Type
            19, // Size
            0, // Date/Time format
            false, // Is upload field
            '`bill_date`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->bill_date->InputTextType = "text";
        $this->bill_date->Nullable = false; // NOT NULL field
        $this->bill_date->Required = true; // Required field
        $this->bill_date->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
        $this->bill_date->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['bill_date'] = &$this->bill_date;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`jdh_patient_bill`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->bill_id->setDbValue($conn->lastInsertId());
            $rs['bill_id'] = $this->bill_id->DbValue;
        }
        return $success;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['bill_id']) && !EmptyValue($this->bill_id->CurrentValue)) {
                $rs['bill_id'] = $this->bill_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->bill_description->DbValue = $row['bill_description'];
        $this->bill_date->DbValue = $row['bill_date'];
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->bill_id->CurrentValue = $keys[0];
            } else {
                $this->bill_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            ...
            $val = !EmptyValue($this->bill_id->OldValue) && !$current ? $this->bill_id->OldValue : $this->bill_id->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("jdhpatientbilllist");
    }
    ...
    {
        global $Language;
        if ($pageName == "jdhpatientbillview") {
            return $Language->phrase("View");
        } elseif ($pageName == "jdhpatientbilledit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "jdhpatientbilladd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "JdhPatientBillView";
            case Config("API_ADD_ACTION"):
                return "JdhPatientBillAdd";
            case Config("API_EDIT_ACTION"):
                return "JdhPatientBillEdit";
            case Config("API_DELETE_ACTION"):
                return "JdhPatientBillDelete";
            case Config("API_LIST_ACTION"):
                return "JdhPatientBillList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jdhpatientbillview", $parm);
        } else {
            $url = $this->keyUrl("jdhpatientbillview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "jdhpatientbilladd?" . $parm;
        } else {
            $url = "jdhpatientbilladd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientbilledit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientbilllist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientbilladd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jdhpatientbilllist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jdhpatientbilldelete");
        }
    }
    ...
    {
        $json = "";
        $json .= "\"bill_id\":" . JsonEncode($this->bill_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->bill_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->bill_id->CurrentValue);
        } else {
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->bill_id->CurrentValue = $key;
            } else {
                $this->bill_id->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        $this->bill_description->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        $this->bill_description->EditValue = $this->bill_description->CurrentValue;
        $this->bill_description->PlaceHolder = RemoveHtml($this->bill_description->caption());

        // bill_date
        $this->bill_date->setupEditAttributes();
        $this->bill_date->EditValue = FormatDateTime($this->bill_date->CurrentValue, $this->bill_date->formatPattern());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->bill_description);
                    $doc->exportCaption($this->bill_date);
                } else {
                    $doc->exportCaption($this->bill_id);
                    $doc->exportCaption($this->patient_id);
                    $doc->exportCaption($this->bill_date);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->bill_description);
                        $doc->exportField($this->bill_date);
                    } else {
                        $doc->exportField($this->bill_id);
                        $doc->exportField($this->patient_id);
                        $doc->exportField($this->bill_date);
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM `jdh_patient_bill`";
